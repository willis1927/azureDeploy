...
import { monitoringDataPointSerializer, collectionConfigurationInfoDeserializer, serviceErrorDeserializer, monitoringDataPointArraySerializer, } from "../models/models.js";
import { expandUrlTemplate } from "../static-helpers/urlTemplate.js";
import { createRestError, operationOptionsToRequestParameters, } from "@azure-rest/core-client";
export function _publishSend(context, ikey, options = { requestOptions: {} }) {
    const path = expandUrlTemplate("/QuickPulseService.svc/post{?api%2Dversion,ikey}", {
...
export async function _publishDeserialize(result) {
...
export async function publish(context, ikey, options = { requestOptions: {} }) {
    const result = await _publishSend(context, ikey, options);
    return _publishDeserialize(result);
}
export function _isSubscribedSend(context, ikey, options = { requestOptions: {} }) {
    const path = expandUrlTemplate("/QuickPulseService.svc/ping{?api%2Dversion,ikey}", {
        "api%2Dversion": context.apiVersion ?? "2024-04-01-preview",
        ikey: ikey,
    }, {
        allowReserved: options?.requestOptions?.skipUrlEncoding,
...
export async function _isSubscribedDeserialize(result) {
    const expectedStatuses = ["200"];
    if (!expectedStatuses.includes(result.status)) {
        const error = createRestError(result);
        error.details = serviceErrorDeserializer(result.body);
...
export async function isSubscribed(context, ikey, options = { requestOptions: {} }) {
